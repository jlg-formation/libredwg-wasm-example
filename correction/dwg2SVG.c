...
#else
        && (strstr(style->font_file, ".ttf") || strstr(style->font_file, ".TTF"))
#endif
    )
    {
#ifdef HAVE_STRCASESTR
...
#else
        if ((strstr(style->font_file, "arial")) || strstr(style->font_file, "Arial"))
#endif
        {
            fontfamily = "Arial";
        }
...
output_LINE(Dwg_Object *obj)
{
    ...
    if (isnan_3BD(line->start) || isnan_3BD(line->end) || isnan_3BD(line->extrusion) || entity_invisible(obj))
        return;
...
output_XLINE(Dwg_Object *obj)
{
    ...
    if (isnan_3BD(xline->point) || isnan_3BD(xline->vector) || entity_invisible(obj))
        return;
...
output_CIRCLE(Dwg_Object *obj)
{
    ...
    if (isnan_3BD(circle->center) || isnan_3BD(circle->extrusion) || isnan(circle->radius) || entity_invisible(obj))
        return;
...
output_ARC(Dwg_Object *obj)
{
    ...
    if (isnan_3BD(arc->center) || isnan_3BD(arc->extrusion) || isnan(arc->radius) || isnan(arc->start_angle) || isnan(arc->end_angle) || entity_invisible(obj))
        return;
...
output_ELLIPSE(Dwg_Object *obj)
{
    ...
    if (isnan_3BD(ell->center) || isnan_3BD(ell->extrusion) || isnan_3BD(ell->sm_axis) || isnan(ell->axis_ratio) || isnan(ell->start_angle) || isnan(ell->end_angle) || entity_invisible(obj))
        return;
...
output_SOLID(Dwg_Object *obj)
{
    ...
    if (isnan_2BD(s1) || isnan_2BD(s2) || isnan_2BD(s3) || isnan_2BD(s4) || entity_invisible(obj))
        return;
...
output_3DFACE(Dwg_Object *obj)
{
    ...
    if (isnan_3BD(ent->corner1) || isnan_3BD(ent->corner2) || isnan_3BD(ent->corner3) || isnan_3BD(ent->corner4) || entity_invisible(obj))
        return;
    printf("\t<!-- 3dface-%d -->\n", obj->index);
    if (ent->invis_flags)
    {
...
output_INSERT(Dwg_Object *obj)
{
    ...
    if (insert->block_header && insert->block_header->handleref.value)
    {
        ...
        if (isnan_3BD(insert->ins_pt) || isnan_3BD(insert->extrusion) || isnan(insert->rotation) || isnan_3BD(insert->scale))
            return;
...
int main(int argc, char *argv[])
{
...
#ifdef HAVE_GETOPT_LONG
    int option_index = 0;
    static struct option long_options[] = {{"verbose", 1, &opts, 1}, // optional
                                           {"mspace", 0, 0, 0},
                                           {"force-free", 0, 0, 0},
                                           {"help", 0, 0, 0},
                                           {"version", 0, 0, 0},
    ...
    while
#ifdef HAVE_GETOPT_LONG
        ((c = getopt_long(argc, argv, ":v:m::h", long_options, &option_index)) != -1)
#else
