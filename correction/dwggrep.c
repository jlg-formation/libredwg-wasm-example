...
match_TABLE(const char *restrict filename, const Dwg_Object *restrict obj)
{
  ...
  for (i = 0; i < _obj->num_cells; i++)
  {
    ...
    else if (_obj->cells[i].type == 2 && _obj->cells[i].additional_data_flag == 1 && _obj->cells[i].num_attr_defs)
    {
                             ...
                             const Dwg_Object *restrict obj)
{
  char *text;
  int found = 0;
  const Dwg_Object_ASSOCMLEADERACTIONBODY *_obj = obj->tio.object->tio.ASSOCMLEADERACTIONBODY;
                                      ...
                                      const Dwg_Object *restrict obj)
{
  char *text;
  int found = 0;
  const Dwg_Object_ASSOC3POINTANGULARDIMACTIONBODY *_obj = obj->tio.object->tio.ASSOC3POINTANGULARDIMACTIONBODY;
  MATCH_AcDbAssocParamBasedActionBody(ASSOC3POINTANGULARDIMACTIONBODY) return found;
}
static int
                                ...
                                const Dwg_Object *restrict obj)
{
  char *text;
  int found = 0;
  const Dwg_Object_ASSOCALIGNEDDIMACTIONBODY *_obj = obj->tio.object->tio.ASSOCALIGNEDDIMACTIONBODY;
                                 ...
                                 const Dwg_Object *restrict obj)
{
  char *text;
  int found = 0;
  const Dwg_Object_ASSOCORDINATEDIMACTIONBODY *_obj = obj->tio.object->tio.ASSOCORDINATEDIMACTIONBODY;
  MATCH_AcDbAssocParamBasedActionBody(ASSOCORDINATEDIMACTIONBODY) return found;
}
                                ...
                                const Dwg_Object *restrict obj)
{
  char *text;
  int found = 0;
  const Dwg_Object_ASSOCROTATEDDIMACTIONBODY *_obj = obj->tio.object->tio.ASSOCROTATEDDIMACTIONBODY;
                                  ...
                                  const Dwg_Object *restrict obj)
{
  char *text;
  int found = 0;
  const Dwg_Object_ASSOCPATCHSURFACEACTIONBODY *_obj = obj->tio.object->tio.ASSOCPATCHSURFACEACTIONBODY;
  MATCH_AcDbAssocParamBasedActionBody(ASSOCPATCHSURFACEACTIONBODY) return found;
}
static int
                                  ...
                                  const Dwg_Object *restrict obj)
{
  char *text;
  int found = 0;
  const Dwg_Object_ASSOCPLANESURFACEACTIONBODY *_obj = obj->tio.object->tio.ASSOCPLANESURFACEACTIONBODY;
  MATCH_AcDbAssocParamBasedActionBody(ASSOCPLANESURFACEACTIONBODY) return found;
}
static int
                                   ...
                                   const Dwg_Object *restrict obj)
{
  char *text;
  int found = 0;
  const Dwg_Object_ASSOCEXTENDSURFACEACTIONBODY *_obj = obj->tio.object->tio.ASSOCEXTENDSURFACEACTIONBODY;
  MATCH_AcDbAssocParamBasedActionBody(ASSOCEXTENDSURFACEACTIONBODY) return found;
}
static int
                                     ...
                                     const Dwg_Object *restrict obj)
{
  char *text;
  int found = 0;
  const Dwg_Object_ASSOCEXTRUDEDSURFACEACTIONBODY *_obj = obj->tio.object->tio.ASSOCEXTRUDEDSURFACEACTIONBODY;
  MATCH_AcDbAssocParamBasedActionBody(ASSOCEXTRUDEDSURFACEACTIONBODY) return found;
}
static int
                                   ...
                                   const Dwg_Object *restrict obj)
{
  char *text;
  int found = 0;
  const Dwg_Object_ASSOCFILLETSURFACEACTIONBODY *_obj = obj->tio.object->tio.ASSOCFILLETSURFACEACTIONBODY;
  MATCH_AcDbAssocParamBasedActionBody(ASSOCFILLETSURFACEACTIONBODY) return found;
}
static int
                                   ...
                                   const Dwg_Object *restrict obj)
{
  char *text;
  int found = 0;
  const Dwg_Object_ASSOCLOFTEDSURFACEACTIONBODY *_obj = obj->tio.object->tio.ASSOCLOFTEDSURFACEACTIONBODY;
  MATCH_AcDbAssocParamBasedActionBody(ASSOCLOFTEDSURFACEACTIONBODY) return found;
}
static int
                                    ...
                                    const Dwg_Object *restrict obj)
{
  char *text;
  int found = 0;
  const Dwg_Object_ASSOCNETWORKSURFACEACTIONBODY *_obj = obj->tio.object->tio.ASSOCNETWORKSURFACEACTIONBODY;
  MATCH_AcDbAssocParamBasedActionBody(ASSOCNETWORKSURFACEACTIONBODY) return found;
}
static int
                                   ...
                                   const Dwg_Object *restrict obj)
{
  char *text;
  int found = 0;
  const Dwg_Object_ASSOCOFFSETSURFACEACTIONBODY *_obj = obj->tio.object->tio.ASSOCOFFSETSURFACEACTIONBODY;
  MATCH_AcDbAssocParamBasedActionBody(ASSOCOFFSETSURFACEACTIONBODY) return found;
}
static int
                                     ...
                                     const Dwg_Object *restrict obj)
{
  char *text;
  int found = 0;
  const Dwg_Object_ASSOCREVOLVEDSURFACEACTIONBODY *_obj = obj->tio.object->tio.ASSOCREVOLVEDSURFACEACTIONBODY;
  MATCH_AcDbAssocParamBasedActionBody(ASSOCREVOLVEDSURFACEACTIONBODY) return found;
}
static int
                                  ...
                                  const Dwg_Object *restrict obj)
{
  char *text;
  int found = 0;
  const Dwg_Object_ASSOCSWEPTSURFACEACTIONBODY *_obj = obj->tio.object->tio.ASSOCSWEPTSURFACEACTIONBODY;
  MATCH_AcDbAssocParamBasedActionBody(ASSOCSWEPTSURFACEACTIONBODY) return found;
}
static int
                                 ...
                                 const Dwg_Object *restrict obj)
{
  char *text;
  int found = 0;
  const Dwg_Object_ASSOCTRIMSURFACEACTIONBODY *_obj = obj->tio.object->tio.ASSOCTRIMSURFACEACTIONBODY;
  MATCH_AcDbAssocParamBasedActionBody(ASSOCTRIMSURFACEACTIONBODY) return found;
}
...
#define ELSEMATCH(OBJECT)                       \
    ...
    if (obj->fixedtype == DWG_TYPE_DICTIONARY || obj->fixedtype == DWG_TYPE_DICTIONARYWDFLT)
                   ...
                   Dwg_Object_Ref *restrict ref)
{
  ...
  if (!ref)
    return 0;
  obj = hdr = ref->obj;
  if (!hdr || hdr->supertype != DWG_SUPERTYPE_OBJECT || hdr->type != DWG_TYPE_BLOCK_HEADER)
    return 0;
       ...
       obj = get_next_owned_entity(hdr, obj)) // without subentities
  {
    ...
    if (!opt_text)
    {
      if (obj->type == DWG_TYPE_DIMENSION_ORDINATE || obj->type == DWG_TYPE_DIMENSION_LINEAR || obj->type == DWG_TYPE_DIMENSION_ALIGNED || obj->type == DWG_TYPE_DIMENSION_ANG3PT || obj->type == DWG_TYPE_DIMENSION_ANG2LN || obj->type == DWG_TYPE_DIMENSION_RADIUS || obj->type == DWG_TYPE_DIMENSION_DIAMETER)
        found += match_DIMENSION(filename, obj);
      ELSEMATCH(VIEWPORT)
      else if (obj->type == DWG_TYPE__3DSOLID || obj->type == DWG_TYPE_BODY || obj->type == DWG_TYPE_REGION)
...
#ifdef HAVE_GETOPT_LONG
  int option_index = 0;
  static struct option long_options[] = {{"case", 0, 0, 'i'}, {"extended", 0, 0, 'x'}, {"count", 0, 0, 'c'}, {"no-filename", 0, 0, 'h'}, {"recursive", 0, 0, 'r'}, {"recursive", 0, 0, 'R'}, {"type", 1, 0, 'y'}, {"dxf", 1, 0, 'd'}, {"text", 0, 0, 't'}, {"blocks", 0, 0, 'b'}, {"tables", 0, 0, 0}, {"help", 0, 0, 0}, {"version", 0, 0, 0}, {NULL, 0, NULL, 0}};
#endif
